...
#define NOTIFY24XX_FLAGS_REQ_ELS_VALID	BIT_2
#define NOTIFY24XX_FLAGS_N2N_PRLI	BIT_3
#define NOTIFY24XX_FLAGS_PN_VALID	BIT_4
	uint16_t srr_rx_id;
	uint16_t status;
...
typedef struct {
	...
	union {
		...
		struct {
			uint16_t wd3_lo; /* PRLI word 3 bit0-15 */
			uint8_t resv0[6];
		} prli;
		struct {
			uint8_t port_id[3];
	...
	uint16_t reserved_7;
	uint16_t ox_id;
} __attribute__((packed)) notify83xx_entry_t;
