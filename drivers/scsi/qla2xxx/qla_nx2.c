...
qla8044_idc_lock(struct qla_hw_data *ha)
{
	...
	while (status == 0) {
		...
		if (++timeout >=
			...
			ql_log(ql_log_warn, vha, 0xb114, "%s: Lock by func"
			    "%d failed after 2s, lock held by func %d,"
			    "lock count %d,"
			    "first_owner %d\n",
...
qla8044_idc_unlock(struct qla_hw_data *ha)
{
	...
	if ((id & 0xFF) != ha->portnum) {
		ql_log(ql_log_warn, vha, 0xb118, "%s: "
		    "IDC Unlock by %d failed, lock owner is %d!\n",
...
exit_poll_reg:
	if (timeout_error) {
		...
		ql_log(ql_log_fatal, vha, 0xb090, "%s: Poll Failed:"
		...
		if (delay) {
			if (qla8044_poll_reg(vha,
				...
				ql_dbg(ql_dbg_p3p, vha, 0xb091,
				    ...
				    __func__);
				ql_dbg(ql_dbg_p3p, vha, 0xb092, "item_num %d, "
				    "entry_num %d\n", i,
	...
	struct qla8044_reset_entry_hdr *p_hdr)
{
	...
	if (vha->reset_tmplt.seq_error == 0) {
		ql_dbg(ql_dbg_p3p, vha, 0xb093, "%s: Reset sequence completed "
		    "SUCCESSFULLY.\n", __func__);
	} else {
		ql_log(ql_log_fatal, vha, 0xb094, "%s: Reset sequence"
		    "completed with some timeout errors.\n", __func__);
	}
}
	...
	char *p_buff)
{
	...
	for ( ; (!vha->reset_tmplt.seq_end) && (index  < entries); index++) {
		...
		case OPCODE_POLL_READ_LIST:
			...
			break;
		default:
			ql_log(ql_log_fatal, vha, 0xb097, "%s: "
			    "Unknown command ==> 0x%04x on "
	...
	uint32_t flash_addr, uint8_t *p_data, int u32_word_count)
{
	...
	if (ret_val != QLA_SUCCESS) {
		ql_log(ql_log_fatal, vha, 0xb09c, "%s: failed to "
		    "write addr 0x%x to FLASH_DIRECT_WINDOW!\n",
		    __func__, addr);
		goto exit_lockless_read;
	}
	...
	if ((flash_offset + (u32_word_count * sizeof(uint32_t))) >
		...
		for (i = 0; i < u32_word_count; i++) {
			...
			if (ret_val != QLA_SUCCESS) {
				ql_log(ql_log_fatal, vha, 0xb09d, "%s: "
				    "failed to read addr 0x%x!\n",
				    __func__, addr);
				goto exit_lockless_read;
			}
			*(__le32 *)p_data  = le32_to_cpu(u32_word);
			...
			if (flash_offset > (QLA8044_FLASH_SECTOR_SIZE - 1)) {
				...
				ret_val = qla8044_wr_reg_indirect(vha,
				    QLA8044_FLASH_DIRECT_WINDOW, (addr));
				if (ret_val != QLA_SUCCESS) {
					ql_log(ql_log_fatal, vha, 0xb09f, "%s: "
					    "failed to write addr "
...
qla8044_copy_bootloader(struct scsi_qla_host *vha)
{
	...
	ql_dbg(ql_dbg_p3p, vha, 0xb0aa, "%s: "
	    "Wrote F/W (size %d) to MS !!!\n",
	    __func__, size);
...
qla8044_need_reset_handler(struct scsi_qla_host *vha)
{
	...
	do {
		if (time_after_eq(jiffies, reset_timeout)) {
			ql_log(ql_log_info, vha, 0xb0c4,
			    "%s: Function %d: Reset Ack Timeout!,"
	...
	if (drv_state != drv_active) {
		ql_log(ql_log_info, vha, 0xb0c7,
		    "%s(%ld): Function %d turning off drv_active "
		    "of non-acking function 0x%x\n", __func__,
		    vha->host_no, ha->portnum, (drv_active ^ drv_state));
...
qla8044_watchdog(struct scsi_qla_host *vha)
{
	...
	if (!(test_bit(ABORT_ISP_ACTIVE, &vha->dpc_flags) ||
		...
		if (qla8044_check_fw_alive(vha)) {
			...
			ql_log(ql_log_warn, vha, 0xb10a,"Firmware hung.\n");
	...
	uint32_t **d_ptr)
{
	...
	uint16_t s_stride, i;
	qla8044_minidump_entry_pollrd_t *pollrd_hdr;
	uint32_t *data_ptr = *d_ptr;

	pollrd_hdr = (qla8044_minidump_entry_pollrd_t *) entry_hdr;
	...
	struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr)
{
	...
	qla8044_minidump_entry_rdmux2_t *rdmux2_hdr;
	uint32_t *data_ptr = *d_ptr;

	rdmux2_hdr = (qla8044_minidump_entry_rdmux2_t *) entry_hdr;
	...
	uint32_t **d_ptr)
{
	...
	qla8044_minidump_entry_pollrdmwr_t *poll_hdr;
	uint32_t *data_ptr = *d_ptr;

	poll_hdr = (qla8044_minidump_entry_pollrdmwr_t *) entry_hdr;
    ...
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr)
{
	...
	qla8044_minidump_entry_rddfe_t *rddfe;
	rddfe = (qla8044_minidump_entry_rddfe_t *) entry_hdr;
    ...
    struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr)
{
	...
	rdmdio = (qla8044_minidump_entry_rdmdio_t *) entry_hdr;
		...
		struct qla8044_minidump_entry_hdr *entry_hdr, uint32_t **d_ptr)
{
	...
	pollwr_hdr = (qla8044_minidump_entry_pollwr_t *) entry_hdr;
...
qla8044_collect_md_data(struct scsi_qla_host *vha)
{
	...
	for (i = 0; i < num_entry_hdr; i++) {
		...
		case QLA8044_POLLRDMWR:
			...
			break;
		case QLA8044_RDDFE:
			rval = qla8044_minidump_process_rddfe(vha, entry_hdr, &data_ptr);
			if (rval != QLA_SUCCESS)
				qla8044_mark_entry_skipped(vha, entry_hdr, i);
			break;
		case QLA8044_RDMDIO:
			rval = qla8044_minidump_process_rdmdio(vha, entry_hdr, &data_ptr);
			if (rval != QLA_SUCCESS)
				qla8044_mark_entry_skipped(vha, entry_hdr, i);
			break;
		case QLA8044_POLLWR:
			rval = qla8044_minidump_process_pollwr(vha, entry_hdr, &data_ptr);
			if (rval != QLA_SUCCESS)
				qla8044_mark_entry_skipped(vha, entry_hdr, i);
			break;
		case QLA82XX_RDNOP:
